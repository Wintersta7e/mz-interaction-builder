  ...
  const handleOpen = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Invalid document structure: missing version, nodes, or edges",
        );
      }
      ...
      for (const node of doc.nodes) {
        if (!node.id || !node.type || !node.position || !node.data) {
          throw new Error(
            `Invalid node: missing required fields (id, type, position, or data)`,
          );
        }
      }
      ...
      for (const edge of doc.edges) {
        if (!edge.id || !edge.source || !edge.target) {
          throw new Error(
            `Invalid edge: missing required fields (id, source, or target)`,
          );
        }
      }
