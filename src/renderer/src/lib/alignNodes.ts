 ...
 */
export function alignNodes(nodes: InteractionNode[], mode: AlignMode): InteractionNode[] {
  ...
  switch (mode) {
    ...
    case "right": {
      const maxRight = Math.max(...nodes.map((n) => n.position.x + getWidth(n)));
      for (const n of result) {
    ...
    case "bottom": {
      const maxBottom = Math.max(...nodes.map((n) => n.position.y + getHeight(n)));
      for (const n of result) {
 ...
 * Returns a new array with updated positions (input is not mutated).
 */
export function distributeNodes(nodes: InteractionNode[], mode: DistributeMode): InteractionNode[] {
