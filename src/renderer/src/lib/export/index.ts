...
export function exportToMZCommands(document: InteractionDocument): MZCommand[] {
  ...
  function processNode(nodeId: string, indent: number): void {
    ...
    switch (node.type) {
      ...
      case 'menu':
        generateMenuCommands(node, indent, edgesBySource, processNode, convergenceNodes, needsLabel)
      ...
      case 'condition':
        generateConditionCommands(node, indent, edgesBySource, processNode, convergenceNodes, needsLabel)
  ...
  function generateMenuCommands(
    ...
    if (hasConditions) {
      ...
      generateDynamicMenuCommands(node, indent, choices, choiceTargets, data, processNode, convergenceNodes, needsLabel)
    } else {
      // Use standard Show Choices command
      generateStaticMenuCommands(node, indent, choices, choiceTargets, data, processNode, convergenceNodes, needsLabel)
    }
  }
  ...
  function generateStaticMenuCommands(
    ...
    choices.forEach((choice, index) => {
      ...
      commands.push({ code: EVENT_CODES.END, indent: indent + 1, parameters: [] })
    })
  ...
  function generateDynamicMenuCommands(
    ...
    choices.forEach((choice, index) => {
      ...
      if (choice.hideCondition) {
        ...
        if (choice.disableCondition) {
          ...
          initLines.push(`  const _t = (${disableScript}) ? '\\\\C[8]${choiceText}\\\\C[0]' : '${choiceText}';`)
    ...
    initLines.push(`$gameMessage.setChoiceCallback(n => { $gameVariables.setValue(99, _mzib_m[n] ?? -1); });`)
    ...
    choices.forEach((_choice, index) => {
      ...
      commands.push({ code: EVENT_CODES.END, indent: indent + 1, parameters: [] })
      commands.push({ code: EVENT_CODES.CONDITIONAL_END, indent, parameters: [] })
    })
  }
          ...
          if (action.switchId !== undefined) {
            ...
            switch (action.switchValue) {
              ...
              case 'toggle':
                ...
                console.warn('Toggle not supported in direct export, defaulting to ON')
                operation = 0
                break
              default:
                operation = 0
