...
describe("alignNodes", () => {
  const nodes = [makeNode("a", 10, 20), makeNode("b", 100, 50), makeNode("c", 200, 80)];
  ...
  it("aligns right — all right edges match rightmost", () => {
    ...
    expect(result.every((n) => n.position.x + (n.measured?.width ?? 180) === 380)).toBe(true);
  });
  ...
  it("aligns bottom — all bottom edges match bottommost", () => {
    ...
    expect(result.every((n) => n.position.y + (n.measured?.height ?? 80) === 160)).toBe(true);
  });
  ...
  it("handles nodes with different measured sizes", () => {
    const mixed = [makeNode("a", 0, 0, 100, 50), makeNode("b", 200, 100, 300, 120)];
...
describe("distributeNodes", () => {
  it("distributes horizontally — even spacing between extremes", () => {
    ...
    const xs = result.sort((a, b) => a.position.x - b.position.x).map((n) => n.position.x);
    expect(xs[0]).toBe(0);
  ...
  it("distributes vertically — even spacing between extremes", () => {
    const nodes = [makeNode("a", 0, 0), makeNode("b", 0, 90), makeNode("c", 0, 300)];
    const result = distributeNodes(nodes, "vertical");
    const ys = result.sort((a, b) => a.position.y - b.position.y).map((n) => n.position.y);
    expect(ys[0]).toBe(0);
  ...
  it("does not mutate input nodes", () => {
    const nodes = [makeNode("a", 0, 0), makeNode("b", 50, 0), makeNode("c", 300, 0)];
    const original = nodes.map((n) => ({ ...n, position: { ...n.position } }));
