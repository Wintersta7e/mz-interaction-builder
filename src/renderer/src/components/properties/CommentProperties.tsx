...
export function CommentProperties({ node, updateNode }: CommentPropertiesProps): React.JSX.Element {
  const data = node.data as CommentNodeData;

  return (
    <div className="space-y-4">
      <div>
        <label className="mb-1 block text-xs text-muted-foreground">Note Text</label>
        <DebouncedTextarea
          value={data.text || ""}
          onChange={(value) => updateNode(node.id, { data: { ...data, text: value } })}
