...
export function setupProjectHandlers(ipcMain: IpcMain): void {
    ...
    async (_event, path: string): Promise<{ valid: boolean; error?: string }> => {
      try {
        ...
        const projectFile = files.find(
          (f) => f.toLowerCase().endsWith('.rmmzproject') || f.toLowerCase().endsWith('.rpgproject')
        )
        if (!projectFile) {
          return {
            valid: false,
        ...
        if (!existsSync(join(path, 'data'))) {
          return { valid: false, error: 'Not a valid RPG Maker MZ project (data folder not found)' }
        }
        return { valid: true }
      } catch (error) {
  ...
  // Set project path
  ipcMain.handle('project:set-path', async (_event, path: string): Promise<void> => {
    projectPath = path
  })
  ...
  ipcMain.handle('project:get-maps', async (): Promise<MZMapInfo[] | { error: string }> => {
    ...
    try {
      ...
      return mapInfos.filter((m: MZMapInfo | null) => m).map((m: MZMapInfo) => ({ id: m.id, name: m.name }))
    } catch (error) {
    ...
    async (_event, mapId: number): Promise<MZMapEvent[] | { error: string }> => {
      ...
      try {
        ...
        return mapData.events
          .filter((e: { id: number; name: string; pages: unknown[] } | null) => e)
  ...
  ipcMain.handle('project:get-switches', async (): Promise<MZSwitch[] | { error: string }> => {
    ...
    try {
      ...
      return system.switches
        .map((name: string, index: number) => ({ id: index, name: name || '' }))
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const mapFile = join(projectPath, 'data', `Map${String(options.mapId).padStart(3, '0')}.json`)
      if (!existsSync(mapFile)) {
        return { success: false, error: `Map file not found: Map${String(options.mapId).padStart(3, '0')}.json` }
      }
      ...
      try {
        ...
        if (!mapEvent) {
          return { success: false, error: `Event ID ${options.eventId} not found in map` }
        }

        const page = mapEvent.pages[options.pageIndex]
        if (!page) {
          return { success: false, error: `Page ${options.pageIndex} not found in event` }
        }
