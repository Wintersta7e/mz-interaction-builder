...
- [Exporting](#exporting)
- [Dialogue Preview](#dialogue-preview)
- [Templates](#templates)
...
Test your interaction flow without leaving the editor. The preview engine walks through your graph, simulating dialogue, choices, and script execution.

### Starting a Preview

- Press **F5** or click the **Preview** button in the toolbar to open the preview panel
- Right-click any node and select **"Preview from here"** to start from a specific point

### Preview Controls

| Control | Action |
|---------|--------|
| **F5** | Toggle preview panel open/close |
| **Space** | Step to the next node |
| **1-9** | Pick a choice by number (when a choice menu is displayed) |
| **R** | Restart the preview from the beginning |
| **Esc** | Close the preview panel |

You can also click the step/restart buttons in the preview panel toolbar, or click choices directly in the dialogue window.

### Preview Components

**Dialogue Window** — Displays text messages with a typewriter effect and shows clickable choice buttons when a Menu node is reached.

**Variable Inspector** — Shows all variables and switches referenced in your graph. Values are editable — change them mid-preview to test different branches. When an RPG Maker project is loaded, variable/switch names are shown alongside IDs.

**Execution Log** — A color-coded transcript of every step taken. Click any entry to navigate to that node on the canvas. Use the **Copy Log** button to copy the transcript.

### Coverage Overlay

After running a preview, nodes on the canvas show coverage badges:

- **Green check** — Node was visited during preview
- **Gray dash** — Node was not visited

Coverage accumulates across multiple preview runs, so you can test different paths and see which nodes still haven't been reached. The coverage percentage is shown in the preview panel header.

### Script Execution

The preview engine evaluates Script actions and Script conditions using a sandboxed environment with mock RPG Maker objects (`$gameSwitches`, `$gameVariables`, `$gameMessage`). Values set during preview are tracked in the Variable Inspector.

### Muted Nodes

Muted nodes are automatically skipped during preview, just like during export. The preview follows the muted node's outgoing connection as if the node wasn't there.

---

## Templates

Save frequently used node patterns as reusable templates.

### Saving a Template

1. Select one or more nodes (use rectangle selection or Ctrl+click for multi-select)
2. Right-click and choose **"Save as Template"**, or press **Ctrl+C** followed by the template save button in the Properties panel
3. Give your template a name and optional description
4. Click **Save**

Templates preserve the selected nodes, their properties, and any edges between them.

### Using a Template

Drag a template from the **Templates** section in the left palette onto the canvas. The nodes are instantiated with new IDs at the drop position, and internal edges are recreated.

### Managing Templates

Templates are stored locally and persist across sessions. You can delete templates from the palette by hovering over them and clicking the delete button.
...
| Shortcut | Action |
|----------|--------|
| `F5` | Toggle preview panel |
| `Space` | Step to next node |
| `1`-`9` | Pick choice by number (during choice menu) |
| `R` | Restart preview |
